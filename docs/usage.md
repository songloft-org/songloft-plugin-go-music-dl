# Songloft Plugin: Go Music DL — 详细文档
...
「我的歌单」页右上角新增 **同步歌单** 入口，可把 go-music-dl 已登录账号的歌单手动同步到 Songloft：

- **范围自选**：面板内列出账号下全部歌单（含「我喜欢」），支持 **全选 / 仅未同步 / 逐张勾选**；
- **增量幂等**：首次同步导入整张歌单并记住绑定；再次同步只补新增的歌（按 `dedup_key` 精确 diff），重复点同步零副作用；用户手动添加进绑定歌单的其他歌**不会被触碰**；
- **失败可续传**：每张歌单以「begin + 循环 step」分步协议同步，后端每步只推进一小批工作并返回实时进度，单步远短于网关超时上限；中途失败/中断后再点一次同步自动接着做（幂等，已导入部分不丢）；单张失败不阻断后续；
- **实时进度**：同步时每张歌单行内显示进度条与阶段文案——「正在读取远端歌单（第 p/P 页 · 已读 n 首）→ 正在比对本地曲库 → 正在校验并导入 m/total（新增 x · 失效 y）」，大歌单同步全程可感知；
- **源支持**：网易云 / QQ / 酷狗 / 汽水（go-music-dl 侧需先在网页端扫码登录，cookie 失效时同步会给出明确提示）。

引擎与绑定存储在插件后端 `src/sync.ts`（绑定仅存映射元数据，diff 真源是宿主歌单内容本身）；删除跟随、定时自动同步、反向同步暂不支持。
